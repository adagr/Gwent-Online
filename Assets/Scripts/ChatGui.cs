...
public class ChatGui : MonoBehaviour, IChatClientListener
{
    ...
    public void OnGUI()
    {
		...
		hideChat = GUI.Toggle(new Rect(GuiRect.width+10,Screen.height-20,75,30),hideChat,"Hide chat");
		if (hideChat)
			return;

        if (chatClient.State == ExitGames.Client.Photon.Chat.ChatState.Uninitialized)
        {
            //if (GUI.Button(new Rect(0, Screen.height-100, 120, 30), "Connect to chat"))
            if (PhotonNetwork.playerName.CompareTo("green") != 0 || dev)
                chatClient.Connect(ChatAppId, NetworkManager.version, new ExitGames.Client.Photon.Chat.AuthenticationValues(PhotonNetwork.playerName));
            else
            {
                chatClient.Connect(ChatAppId, NetworkManager.version, new ExitGames.Client.Photon.Chat.AuthenticationValues("blue"));
                PhotonNetwork.playerName = "blue";
            }
        }
